...
const STRIPE_SECRET_KEY =
  "sk_test_51Q5MsnKcg0aY6eUJEzI7ZbILm8r4Zr7x188NzJOEZRc8eJ4bSFxY7BkEhRjKNyQy7gqlcQv7OkeNrmC1pe9cr4Rb00VHCKVMyW";
...
router.post("/create-checkout-session", async (req, res) => {
  ...
  if (!cartItems || !Array.isArray(cartItems) || cartItems.length === 0) {
    ...
    return res
      .status(400)
      .json({ error: "Cart items array is missing or invalid" });
  }
  ...
  try {
    ...
    const lineItems = cartItems.map((product) => {
      if (
        ...
        throw new Error(
          "Invalid product data. Ensure each product has name, image, price, and quantity."
        );
      }
      ...
      return {
        price_data: {
          ...
          product_data: {
            name: product.name,
            images: [`https://earthifyy.vercel.app${product.image}`],
          },
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "payment",
      success_url: `https://earthifyy.vercel.app/customer/success?order_id=${orderId}`, // Redirects to the success page
      cancel_url: `https://earthifyy.vercel.app/cancel`, // Redirects to the cancellation page
