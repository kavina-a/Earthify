...
const createProduct = asyncHandler(async (req, res) => {
    try {
      ...
      console.log('Request Body:', req.body); // Log form fields data
      console.log('Uploaded File:', req.file); // Log the uploaded file
  
      // Extract form data
      const { name, description, price, category, countInStock, certifications} = req.body;
      const image = req.file ? `/uploads/${req.file.filename}` : null;
  
      // Validate fields
      if (!name || !description || !price || !countInStock || !image) {
        return res.status(400).json({ error: 'All fields (name, description, price, image, etc.) are required' });
      }
  
      // Assuming that req.user._id is set by your authentication middleware
...
const removeFromFavorites = asyncHandler(async (req, res) => {
  try {
    ...
    if (!customer.favorites.includes(trimmedProductId)) {
      return res.status(400).json({ message: 'Product is not in your favorites' });
    }
